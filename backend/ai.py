...
- **不同事项的图片证据要分开识别**：上传的多张奖状/证书图片如果对应**不同的比赛/证书/加分事项**（如一张蓝桥杯、一张英语竞赛），必须逐张查看、各自提取为独立条目（几张不同事项的奖项图就输出几条 items），不得合并成一条或只输出其中一张。
- **同一比赛的多张奖状只保留一条，取最高分**：若多张图片属于**同一个比赛/证书事项**（如同一赛事的全国+省级证书、同一比赛不同奖次的几张证书），只输出一条 items，`level` 如实列出最高级别（或并列各级别）供定分环节取最高分值，**绝不把同一比赛拆成多条**。
- 文字申报与图片证据都出现的同一加分点只保留一条（避免同一奖项图文重复）。
