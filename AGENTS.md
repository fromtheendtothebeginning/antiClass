...
- **前端交互约定**（2026-09 重构后固化，勿回退）：①登录是**独立弹窗**（`Modal`，非内联表单），入口在顶栏「管理员登录」，**内容只保留 账号/密码/登录/取消**（曾做过左右两栏品牌栏版本，按用户要求精简，勿再引入品牌区/装饰/字段图标）；②`api.js` 的 `request()` 统一拦截 401（`/api/login` 密码错的 401 除外）→ 触发 `setOnAuthExpired` 回调 → App 的 `expireSession()` 清 localStorage 三键、退回榜单、自动弹出带「登录已过期」提示的登录窗（**新增带 token 的请求必须走 `request()`**，否则拿不到该行为）；③挂载时用 `GET /api/me` 验活；④**文件导入走大窗口**：管理 Tab「打开导入窗口」→ `Modal size="lg"` + `DropZone`（拖入/点击即导入；root 必须显式选班级，不再默认第一个班）；第二课堂导入同款但保留显式提交按钮（有阈值参数）；⑤**文本输入一律用 `components/TextField.jsx`**（统一 12px 圆角 + `--bg-elevated` 底 + 聚焦光圈；`forwardRef` + 不包 wrapper + props 全透传，因为多处 flex 尺寸规则挂在 input 本身）；⑥**榜单必须先有班级**：未登录访客若 `localStorage` 里没有 `sel_class_id`，榜单 Tab 显示「请选择要查看的班级」引导卡而不发请求；选过的班存 `sel_class_id`；⑦可复用的 `components/`：`Modal`（`size`/`closeOnOverlay`/自动聚焦，`danger` 时聚焦取消按钮）、`DropZone`（拖拽区，`.compact` 紧凑变体、`.slim` 聊天细条）、`FileChips`（已选文件 chip）、`TextField`、`ThemeToggle`（跟随系统/浅色/深色三态，写 `<html data-theme>` + `localStorage["theme"]`）。
- 安全基线（2026-09 安全审计后固化，勿回退）：证据下载强制 `Content-Disposition: attachment` + 扩展名 Content-Type 白名单 + nosniff；公开上传接口限单文件 10MB、最多 5 个、text ≤2000 字、analyze/class-committee 每 IP 每小时 10 次；数据库凭据在 `backend/db_config.json`（gitignore），勿提交。**上传证据类型黑名单**（`main.py DANGEROUS_EXT`：可执行/脚本/网页/Office 宏等一律 400，`save_upload` 公共入口生效）＋ **AI 内容审核**（`ai.moderate_content`：图片识图+文本抽检，判定涉政/色情/暴恐/赌博/诈骗等违法内容，违规拒收；未配置/调用失败默认放行，审核用于驳回后编辑重提的新证据）。
- **榜单公开可见**（`GET /api/leaderboard`、`GET /api/export` 无需登录，但**必须带 class_id**）；仅管理员可修改（上传 xlsx 需 Bearer token）。前端未登录先选班（见「前端交互约定」⑥）再看榜单，登录后显示导入/调分/管理入口。
...
- **榜单/导出必须显式指定班级（2026-09 修「两班混排」后固化，勿回退）**：`GET /api/leaderboard` 与 `GET /api/export` 的 `class_id` 为空/缺失一律 **400**（detail 中文说明）——此前 `db.list_students(None)` 会 `SELECT * FROM students` 把多个班**交错混排并统一编号名次**，前端 `classSel` 初值为空字符串时首屏就会踩到，且有两处 `loadBoard()` 漏传参（调分后、审批刷新后）会主动把混班榜单灌进页面。配套前端守卫：`loadBoard(cid)` 无 cid 直接清空不发请求、`boardSeq = useRef` 请求序号丢弃过期响应、切班走 `handlePickClass`（清空 + 存 `localStorage["sel_class_id"]`）、表格/datalist 行 key 用 `class_id:sid`（DB 主键是复合的，只用 sid 会 key 冲突）。`db.list_students(None)` 本身保留（分数调整里 root 跨班选人依赖它），但 HTTP 层已无法用它取到合并榜单。
...
- 审批：**申报列表与证据公开可见**（`GET /api/awards`、`GET /api/awards/{id}/evidence/{file}` 无需登录）；`POST /api/awards/{id}/approve`、`reject`、`withdraw`（撤回，撤销已加分并回到待审批）、`delete`（删除记录+证据，若已通过则一并撤销加分）均需 Bearer token。通过时可传 `{points,category}` 覆盖 AI 定分；通过后按 `CATEGORY_FIELD`（德育→deyu…附加分→fujia）累加并**封顶**（板块 100/附加分 5），重算 total 重排榜；同一记录不可重复审批。**驳回必填理由**：`POST .../reject` body `{reason}`（空理由 400），理由存 awards 表 `reject_reason` 列并在卡片红字展示给申报人。**驳回后可编辑重提**：`POST /api/awards/{aid}/edit`（**公开**，10 次/时/IP，multipart `category/points/basis/files[]`，也兼容 JSON 无文件提交；带 Bearer token 时额外校验班级 scope）仅 `approved=驳回` 允许，保存后回「待审批」并清空驳回理由，新证据文件**追加**到原记录（类型黑名单+AI 内容审核双重把关，违规拒收）；待审批/已通过记录 edit 一律 400。前端驳回记录卡片对**所有人**显示「编辑并重新提交」按钮（弹窗内改栏目/分值/依据+预览/追加证据），审核卡片本身字段只读。
- **加分一遍过**（2026-09，`backend/assess.py`）：AI 依据评分办法原文**逐项提问**、学生逐项回答的对话小程序，全程**流式输出**（SSE，`ai.stream_chat`；响应带 `X-Accel-Buffering: no` + 服务器 nginx `proxy_buffering off` 保证逐字到达）。入口在顶部 Tab「加分一遍过」独立页，需先介绍+确认（学号+开始）；每轮 AI 回复以 `==JSON== {...}` 尾行上报新加分项（category/points/basis，后端去重+封顶钳制）与 `done` 结束标记；忽略 JSON 或解析失败时安全降级为纯对话。规则要点：**回复自然问答**（无【要点】【判定】【下一问】标签前缀；先自然确认→给加分结论→顺势问下一项，判完加分不得停；可加分结论含机器句式"本项加分：栏目=…，分值=…，依据=…"供后端兜底提取）；快捷词义 没有/确认/继续（无/有/推进下一项）；栏目归属 AI 自查原文严禁反问；涉及后续栏目的内容先记录、到该栏目统一核定；小节问完自动过渡；班级活动出勤/第二课堂/智育/班委**静默跳过不出现**；**只读注入该生系统已有申报（approved=是）**→ 命中项不再询问且 add 上报按栏目+分值硬去重拒绝（发 warning 防重复加分）。正文有加分判定但 add 漏报 → warning 提示可用**手动添加加分项**补录。**done 后允许一次补充对话**，补充轮结束会话（ended），ended 后再发消息返回 400。加分项可编辑（栏目/分值/依据/删除）并在提交时按条上传证据（multipart `items` JSON + `file_{i}`）。**对话可传图片/文件**：message 为 multipart（`text`+`files[]`），图片经 `ai.image_part` 转 image_url 随当轮发 AI 识图（仅当轮可见，不入历史），也兼容 JSON body（text 单字段）。前端：AI 气泡 Markdown 渲染、聊天窗高度固定自动滚底、快捷按钮 没有/确认/继续、聊天输入附件按钮+待发送 chip、按条证据。端点：`POST /api/assess/start`（Form sid，10 次/时/IP，返回 existing 只读清单）、`/{id}/message`（SSE，30 次/10 分/IP）、`/finish`、`/rewind`（30 次/10 分/IP）、`/submit`（公开，10 次/时/IP）。会话存内存（上限 100、TTL 1h），重启即失。
- **一遍过撤回/回滚（2026-09，勿回退）**：会话新增 `turns`（每轮结束时记 `{msgs,items,done}` 标记）与 `busy`（进行中的轮次计数）。标记由 `main.py` 的 `_gen()` 在 `finally` 里通过 `assess.end_turn(mark=True)` 打——**必须在「流式 + 识图分类」全部写完后打**（分类那批加分项也要计入切片），且仅当本轮真的追加过用户消息才打（否则流结束时失败的一轮会让 `len(turns)` 与真实发言数错位，前端 `keep_users↔轮次` 换算整体偏移）。`assess.rewind(session_id, keep_users)` 在 `_LOCK` 内按标记切片 `messages`/`items`/`turns`，`done` 从该轮标记恢复、`ended` 复位为 False（撤回了就能继续对话，也能撤销 finish），并刷 `updated`；`busy>0` 抛 SessionBusy（端点映射 **409**），越界/非法抛 RewindInvalid（**400**），会话不存在 **404**。前端：**鼠标悬停自己发过的消息**即显示「↺ 撤回」按钮（触屏点气泡固定为 `recall-open` 常驻；`passBusy` 流式期间不渲染），确认弹窗后调 `POST /api/assess/{id}/rewind` JSON `{keep_users}`（= 保留的学生发言条数，**不含开场"开始"轮**，后端 `keep_turns = keep_users + 1`）；响应回传权威 `items` 全量，前端按 `category|Number(points)|basis` 三元组从本地池匹配重建（**不能按下标切片**：`auto_items` 分支的客户端去重会让本地下标与后端错位），并把被撤回那条的原文与附件放回输入框（附件靠前端 `fileObjs` 保存的 File 对象，服务端历史里没有文件引用）。
...
- `GET /api/me` + Bearer token → `{"username","role","class_id","exp"}`（只校验不续期；前端刷新页面时用它验活，401 即视为会话失效）
...
- `GET /api/leaderboard?class_id=<必需>` → `{"students":[{rank,sid,name,course_count,credits,gpa,deyu,score,tiyu,meiyu,laoyu,fujia,total}], "meta":{rows,source,class_id,class_name}}`（score 即智育，total 即综合测评成绩）。**class_id 缺失一律 400**；**榜单只读，不再支持点击改分**
...
- `GET /api/export?class_id=<必需>` → Excel(.xlsx，openpyxl 生成，**9 列**：学号,姓名,德\*15%,智\*60%,体\*10%,美\*5%,劳\*10%,附加,总分，对齐附件 `25-26下学期xx班综合测评总分.xlsx`；文件名 `25-26下学期{班名}班综合测评总分.xlsx`（RFC 5987 编码），挂科学生整行红字且总分为 -1)。**class_id 缺失一律 400**（否则会把合并榜单导出成一个班级名为空的怪文件）
- `GET /api/export/evidence-zip`（+ Bearer token）→ 将**已通过(approved=是)**的申报按人打包 zip：每人一个「学号_姓名/」目录（含该生 `申报明细.yaml` 与 `evidence/` 证据文件），外加总 `index.yaml`；**多人共用的证据只在顶层 `公共证据/` 保留一份**，个人明细里以 `ref` 指向它（不再放副本）。root 可不带 class_id 导出全部班级或指定单班；admin 不传 class_id 时强制导出本班，传他班 403（`check_class_scope`）。zip 在内存 BytesIO 生成后 StreamingResponse（`content-disposition: attachment; filename=evidence_backup_YYYYMMDD.zip`）；无已通过申报返回 400。证据归档名去掉 save_upload 的 32 位 hex 前缀（`_evidence_readable`），同人重名自动加序号；folder 缺失/文件丢失的记录在 yaml 里标 `missing: true` 跳过。yaml 由 `_yaml_dumps` 零依赖序列化生成（Python 3.14 装不上 PyYAML）
